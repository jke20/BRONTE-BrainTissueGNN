...
if (length(args) == 0) {
  cat("Usage: Rscript my_script.R <arg1> <arg2>\n")
  quit(status = 1)
}

# Access and use the arguments
pred_subject <- args[1]     # the subject to predict
...
CP_gtex_subject_region <- function(pred_subject, pred_region, K, prop, conf) {
  ...
  target = Y[predict_subject, predict_region, ]
  Y[predict_subject, predict_region, ] <- NA
  ...
  prediction = Y_predict[predict_subject, predict_region_idx, ]
  ...
  output_file <- paste0(output_path, predict_subject, "_", predict_region, "_", 
                        K, "_", prop, "_", conf, ".csv")
  # Write the data frame to a CSV file
  write.csv(output_data, file = output_file)
}

print(paste0("Subject: ", pred_subject))
print(paste0("Region: ", pred_region))
print(paste0("K: ", K))
print(paste0("prop: ", prop))
print(paste0("conf: ", conf))

CP_gtex_subject_region(pred_subject, pred_region, K, prop, conf)
# Rscript CP_gtex_subject_region.R "GTEX.13O3O" "Putamen_basal_ganglia" 17
