...
from atg_10_model import GO_model
from atg_10_model import lm_general_allen_to_gtex
from atg_10_model import lm_general_gtex_to_gtex
...
def train(args):
    ...
    g_emb_error = 0.035
    g_emb_size = 2 ** 4
    g_emb_path = '/project/pi_rachel_melamed_uml_edu/Jianfeng/Allen/src/Pytorch/01252024/ATG_91_103_30/'
...
#     model = GO_model(GNN_hl1_size, r_emb_size, g_emb_size, MLP_hl1_size, MLP_hl2_size, 
...
#             reg_id = i % n_region
#             r = keys[reg_id]
#             gtex_exp = torch.tensor(summ_gtex_info[subject].values).to(device)
...
#         print(f"Epoch {e + 1}, training mse on {len(unseen_region_list)} unseen regions on 6 allen subjects: {train_e_total_avg};")
...
#         print(f"Epoch {e + 1}, training mse on {len(used_region_list)} used regions on 6 allen subjects: {train_e_total_avg};")


#         #####-----Compute the error of 10 summarized gtex regions on allen subjects-----#####
#         # set model to training mode
#         model.eval()
#         # record all pred and real to compute correlation
#         specific_train_e = 0
...
#                     othername = 'Cerebellum'
#                 elif region=='Frontal_Cortex_BA9':
#                     othername = 'Cortex'
#                 else:
#                     othername = region
#                 targets = torch.tensor(summ_gtex_info[subject].loc[:,region]).to(device)
#                 gen_tuple = torch.tensor(np.arange(N_gene), dtype=torch.long).to(device)
#                 x_reg_exp = gtex_exp.clone().float().to(device)
#                 node_for_region = used_intersected_region_dict[othername]
#                 node_for_region_idx = [keys.index(x) for x in node_for_region]
#                 with torch.no_grad():
#                     pred = model(node_for_region_idx, gen_tuple, x_reg_exp, edge_index, N_gene).reshape(-1).to(device)
#                 loss = criterion(pred, targets)
#                 specific_train_e = specific_train_e + loss.item()
#         # compute the MSE loss
#         specific_train_avg = specific_train_e / (len(all_ids)*len(region_pick))
#         specific_train_avg_loss.append(specific_train_avg)

#         print(f"Epoch {e + 1}, training mse of 10 gtex regions on 6 allen subjects: {specific_train_avg};")


#         #####-----Compute the error of 10 gtex regions on gtex subjects-----#####
...
#                 gen_tuple = torch.tensor(np.arange(N_gene), dtype=torch.long).to(device)
#                 x_reg_exp = torch.tensor(gtex_exp_10.values).float().to(device)        
#                 node_for_region = used_intersected_region_dict[othername]
#                 node_for_region_idx = [keys.index(x) for x in node_for_region]
#                 with torch.no_grad():
#                     pred = model(node_for_region_idx, gen_tuple, x_reg_exp, edge_index, N_gene).reshape(-1).to(device)
...
#         # compute the MSE loss
#         specific_test_avg = specific_test_e / (len(sub_all_ids)*len(region_pick))
#         specific_30_test_avg_loss.append(specific_test_avg)

#         print(f"Epoch {e + 1}, testing mse on 10 gtex regions on 30 gtex subjects: {specific_test_avg};")
...
#     model_name = f'ATG_10_103_epoch_{epoch}_bf_architecture.pth'
#     torch.save(model, data_dir+model_name)
#     weights_name = f'ATG_10_103_epoch_{epoch}_bf_weights.pth'
...
#     ax.set_title(f"GO model performance BEFORE fine-tuning;\n" +
#                  f"testing subjects: 30 gtex subjects;\n" +
#                  f"batch size: {batchsize}; lr = {lr};\n", size=10)
...
#     ax.plot(range(1,1+len(specific_train_avg_loss)), specific_train_avg_loss, color='blue', label="NN: 10 gtex regions on 6 allen")
#     ax.plot(range(1,1+len(specific_30_test_avg_loss)), specific_30_test_avg_loss, color='orange', label="NN: 10 gtex regions on 30 testing gtex")
...
#     result_dir = '/project/pi_rachel_melamed_uml_edu/Jianfeng/Allen/src/Pytorch/02162024/ATG_10_103/Result/'
#     fig_name = f'ATG_10_103_MSEcurve_bf_epoch_{epoch}'
#     plt.savefig(result_dir+fig_name+'.png', dpi=300, bbox_inches='tight')
    ...
    # define the model, optimizer and loss function
    finetuned_model = GO_model(GNN_hl1_size, r_emb_size, g_emb_size, MLP_hl1_size, MLP_hl2_size, 
                               edge_index, dropout, N_gene, N_node, n_node).to(device)
    ...
    # specific training average loss on the allen regions that we used to generate the predicted gtex region
    allen6_10_gtex_loss_af = []
    gtex30_10_gtex_loss_af = []
    ...
    for e in range(f_epoch):
        ...
        #####-----Fine tune the model on 10 gtex regions on 30 gtex subjects-----#####
        # set model to training mode
        ...
        for i in sample_shuffle:
            ...
            if reg_name=='Cerebellar_Hemisphere':
                ...
                othername = reg_name
            reg_id_list = used_intersected_region_dict[othername]
            reg_id_list_idx = [keys.index(x) for x in reg_id_list]
            # x exp mat
            gtex_exp_10 = exp_gtex_dict[subject]
            gtex_exp_target = gtex_exp_10.loc[:,reg_name]
            gtex_exp_10 = torch.tensor(gtex_exp_10.values).to(device)
            targets = torch.tensor(gtex_exp_target.values).to(device)
            # shuffle the genes for training
            gene_shuffle_order = np.random.choice(N_gene, N_gene, replace=False)
            gen_ids = gene_shuffle_order
            targets = targets[gene_shuffle_order]
            gtex_exp = gtex_exp_10[gene_shuffle_order]        
            # batch training
        ...
        #####-----Compute the training error of 10 gtex regions on gtex subjects-----#####
        # set model to testing mode
        ...
        for subject in sub_all_ids:
            ...
            for region in region_pick:
                ...
                node_for_region_idx = [keys.index(x) for x in node_for_region]
                with torch.no_grad():
                    pred = finetuned_model(node_for_region_idx, gen_tuple, x_reg_exp, edge_index, N_gene).reshape(-1).to(device)
                loss = criterion(pred, targets)
                gtex30_f_e = gtex30_f_e + loss.item()
        # compute the MSE loss
        specific_gtex30_avg = gtex30_f_e / (len(sub_all_ids)*len(region_pick))
        gtex30_10_gtex_loss_af.append(specific_gtex30_avg)

        print(f"Model Fine-tuning Epoch {e + 1}, training mse on 10 gtex regions on 30 gtex subjects: {specific_gtex30_avg};")


        #####-----Compute the average error across all unseen allen regions on allen subjects-----#####
        # set model to training mode
        finetuned_model.eval()
        # record all pred and real to compute correlation
        train_e = {}
        ...
        # compute the MSE loss
        specific_train_avg = specific_train_e / (len(all_ids)*len(region_pick))
        allen6_10_gtex_loss_af.append(specific_train_avg)

        print(f"Model Fine-tuning Epoch {e + 1}, 10 gtex regions on 6 allen subjects: {specific_train_avg};")
    ...
    model_name = f'ATG_10_103_af_epoch_{epoch}_finetuning_epoch_{f_epoch}_architecture.pth'
    weights_name = f'ATG_10_103_af_epoch_{epoch}_finetuning_epoch_{f_epoch}_weights.pth'
    #model_name = f'ATG_10_103_freeze_GNN_af_epoch_{epoch}_finetuning_epoch_{f_epoch}_architecture.pth'
    #weights_name = f'ATG_10_103_freeze_GNN_af_epoch_{epoch}_finetuning_epoch_{f_epoch}_weights.pth'
    ...
    allen6_s_pd_min = min(allen6_10_gtex_loss_af)
    allen6_s_pd_min_pos = allen6_10_gtex_loss_af.index(allen6_s_pd_min)+1
    gtex30_s_min = min(gtex30_10_gtex_loss_af)
    gtex30_s_min_pos = gtex30_10_gtex_loss_af.index(gtex30_s_min)+1

    ax.set_title(f"GO model performance AFTER fine-tuning (trainable GNN);\n" + 
                 f"training_epoch: {epoch}; finetuning_epoch: {f_epoch}\n" + 
                 f"batch size: {batchsize}; lr = {lr};\n", size=10)
    ax.set_xlabel("MSE loss")
    ax.plot(range(1,1+e), allen6_unseen_loss_af, color='black', label=f"{len(unseen_region_list)} unseen regions on 6 allen")
    ax.plot(range(1,1+e), allen6_used_loss_af, color='purple', label=f"{len(used_region_list)} used regions on 6 allen")
    ax.plot(range(1,1+e), allen6_10_gtex_loss_af, color='blue', label="10 gtex regions on 6 allen")
    ax.plot(range(1,1+e), gtex30_10_gtex_loss_af, color='orange', label="10 gtex regions on 30 testing gtex")
    ...
    result_dir = '/project/pi_rachel_melamed_uml_edu/Jianfeng/Allen/src/Pytorch/02162024/ATG_10_103/Result/'
    #fig_name = f'ATG_10_103_freeze_GNN_MSEcurve_af_epoch_{epoch}_f_epoch_{f_epoch}'
    fig_name = f'ATG_10_103_MSEcurve_af_epoch_{epoch}_f_epoch_{f_epoch}'
    plt.savefig(result_dir+fig_name+'.png', dpi=300, bbox_inches='tight')
